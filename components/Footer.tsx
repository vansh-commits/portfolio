      ...
      <div className="max-w-6xl mx-auto text-center relative z-10">
        <div className="mb-10 sm:mb-12">
          <div className="flex items-center justify-center mb-4 sm:mb-6">
            <Sparkles className="w-6 h-6 sm:w-6 sm:h-6 text-cyan-400 mr-2 sm:mr-3 animate-pulse" />
            <h3 className="text-4xl sm:text-4xl md:text-5xl font-thin text-white bg-gradient-to-r from-white via-gray-100 to-white bg-clip-text text-transparent">
              Let's Vibe & Build
            </h3>
            <Sparkles className="w-6 h-6 sm:w-6 sm:h-6 text-cyan-400 ml-2 sm:ml-3 animate-pulse" />
          </div>
          <p className="text-lg sm:text-lg md:text-xl text-white/70 mb-6 sm:mb-8 font-light px-1">
          ...
          <div className="text-white/60 text-base sm:text-base md:text-lg">
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 gap-4 sm:gap-6 mb-10 sm:mb-12">
          {funFacts.map((fact, index) => (
            ...
            >
              ...
              <p className="text-white/70 text-xs sm:text-sm font-light">{fact.text}</p>
            </div>
          ))}
        </div>
        ...
        <div className="flex justify-center flex-wrap gap-4 sm:space-x-6 mb-10 sm:mb-12">
          {socialLinks.map(({ icon: Icon, href, label, hoverColor }) => (
            <a
              key={label}
              href={href}
              className={`group p-4 sm:p-5 bg-white/10 hover:bg-white/20 rounded-2xl transition-all duration-500 backdrop-blur-sm border border-white/10 hover:border-white/30 hover:scale-110 hover:shadow-xl hover:shadow-white/10`}
              aria-label={label}
              target='_blank'
            >
              <Icon 
        ...
        <div className="space-y-4">
          ...
          <div className="pt-6 sm:pt-8 border-t border-white/20">
            ...
            <div className="mt-2 text-white/40 text-xs sm:text-sm">
